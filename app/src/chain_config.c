...
// with path, hrp and encoding. The coin type of every entry is accepted by the
// APDU path check (see isSupportedCoinType).
...
bool isSupportedCoinType(uint32_t path) {
  // The two built-in coin types: default Cosmos and the Eth-style family
  if (path == HDPATH_1_DEFAULT || path == HDPATH_ETH_1_DEFAULT) {
    return true;
  }

  // Any coin type declared in the chain configuration. The (coin type, hrp)
  // pair is validated later by checkChainConfig, once the HRP is known.
